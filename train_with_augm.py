...
if os.path.exists(os.path.join(dirc_path, r"data\datasets\meowmeowmeowmeowmeow\gtsrb-german-traffic-sign\versions\1")):
    data_set_apth = os.path.join(dirc_path, r"data\datasets\meowmeowmeowmeowmeow\gtsrb-german-traffic-sign\versions\1")
else:
...
#data augmentation (existing dataset gets completely modified, use with caution)
for i in range(classes):
    ...
    for j, image in enumerate(images):
        ...
        angles = [x for x in range(91) if x%10 == 0]                                  

        for k, angle in enumerate(angles):
            rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)             
            rotated_image = cv2.warpAffine(image, rotation_matrix, (width, height))     #rotating images by angles 10, 20, 30, ..., 90 degrees
    
            cv2.imwrite(path + '/' + f"png{i}_{j}_{k}.png", rotated_image)              #saving as both .png and .jpg
            cv2.imwrite(path + '/' + f"jpg{i}_{j}_{k}.jpg", rotated_image)
            counter += 2

        mean = 0
        std_dev = 25
        
        noise = np.random.normal(mean, std_dev, image.shape).astype(np.uint8)           #adding gaussian noise
        noisy_image = cv2.add(image, noise)
        cv2.imwrite(path + '/' + f"noisypng{i}_{j}.png", noisy_image)        
        cv2.imwrite(path + '/' + f"noisyjpg{i}_{j}.jpg", noisy_image)                   #saving as both .png and .jpg
        counter += 2
...
#saving model
torch.save(model1.state_dict(), "model_with_data_augmentation.pth")
